    ...
    <div class="row gy-5 mb-5 align-items-center">
        ...
        <div class="col-lg-6">
            <div class="card-glass p-3 text-center" style="background: rgba(18, 20, 32, 0.4);">
                <img src="<?php echo BASE_URL; ?>/<?php echo htmlspecialchars($prod['image_url']); ?>" alt="<?php echo htmlspecialchars($prod['name']); ?>" class="img-fluid rounded-4 w-100" style="max-height: 400px; object-fit: contain;" onerror="this.src='<?php echo BASE_URL; ?>/assets/img/placeholder.svg'">
            </div>
        </div>
        ...
        <div class="col-lg-6">
            ...
            <div class="d-flex align-items-center gap-4 mb-4">
                ...
                <?php
                    $rating_data = get_avg_rating($conn, $product_id);
                    $avg = $rating_data['avg'];
                    $cnt = $rating_data['total'];
                ?>
                <?php if ($cnt > 0): ?>
                    <div class="star-rating">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                            <i class="bi <?php echo $i <= round($avg) ? 'bi-star-fill' : 'bi-star'; ?>"></i>
                        <?php endfor; ?>
                        <span class="ms-1 text-secondary small"><?php echo number_format($avg,1); ?> (<?php echo $cnt; ?> reviews)</span>
                    </div>
                <?php else: ?>
                    <div class="text-secondary small"><i class="bi bi-star me-1"></i>No reviews yet</div>
                <?php endif; ?>
